...
def transfer_time_to_weight(dataframe, 
        ...
        for j in range(1, df_copy.shape[1]):
            if i != j - 1:
                ...
                if isinstance(tm, (int, float)):
                    weight = normaliser / tm ** 2 if tm <= threshold else 0
                else:
                    weight = 0
            else:
                weight = max_weight
    ...
    print("done, normalising now")
     # Normalize weights and fill diagonal with 1 (self-weight)
    for i in range(df_copy.shape[0]):
        max_weight = max(df_copy.iloc[i, 1:].values)
        if max_weight != 0:
            df_copy.iloc[i, 1:] = df_copy.iloc[i, 1:] / max_weight
            df_copy.iloc[i, i + 1]  = 1.0
    print("why do we need that?")
    #df_copy.iloc[0, 0] = 1.0

    return df_copy

def transfer_time_to_weight_faster(dataframe, threshold=30):
    """
    Transfer times into weights, with optimized calculations and handling for values less than 1.
    If time_values are less than 1, the weight is set to max_weight before normalization.
    """
    max_weight = 10.0
    normaliser = threshold ** 2
    df_copy = dataframe.copy()

    # Step 1: Extract time values, excluding the first column
    time_values = df_copy.values  

    # Step 2: Compute weights with conditions
    # If time_values < 1, set weight to max_weight; otherwise, apply threshold rule
    weights = np.where(
        time_values < 1, 
        max_weight,  # Condition for time_values < 1
        np.where(time_values <= threshold, normaliser / (time_values ** 2), 0)
    )

    # Set self-weight (diagonal) to max_weight
    np.fill_diagonal(weights, max_weight)

    # Step 3: Normalize weights row-wise (excluding the self-weight)
    max_weights = np.max(weights, axis=1, keepdims=True)
    normalized_weights = np.where(max_weights != 0, weights / max_weights, weights)
    
    # Step 4: Set self-weight to 1 after normalization
    np.fill_diagonal(normalized_weights, 1.0)

    # Step 5: Update the dataframe with normalized weights
    df_copy.iloc[:, :] = normalized_weights
    return df_copy

def vert_to_square(df):  
    pivot_df = df.pivot_table(index='from_id', columns='to_id', values='travel_time')
    
    all_ids = pd.unique(df[['from_id', 'to_id']].values.ravel())
    
    pivot_df = pivot_df.reindex(index=all_ids, columns=all_ids)
    
    # Reset the index so that 'from_id' becomes a regular column
    pivot_df.reset_index(inplace=True)
    
    # Rename the 'from_id' column to 'id'
    pivot_df.rename(columns={'from_id': 'id'}, inplace=True)
    
    # Drop the current index column and set the first column as the new index
    pivot_df.set_index('id', inplace=True)
    #pivot_df.drop(columns="to_id", inplace=True)
    return pivot_df
