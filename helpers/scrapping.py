...
transportation = ['station', 'bus_station', 'tram_stop']
...
def get_place_POI_tags(place: str,
                       tags={"amenity": ["restaurant", "cafe", "bar", "ice_cream",
                                         "fast_food", "pub", "food_court", "biergarten"]},
    ...
    if get_network:
        g_place = ox.graph_from_place(
            place, buffer_dist=1000, network_type=network_type, retain_all=True, truncate_by_edge=True)
        g_place = ox.project_graph(g_place, to_crs="WGS-84")
        if consolidate:
            g_proj = ox.project_graph(g_place)
            g_place = ox.consolidate_intersections(
...
def get_place_POI(place: str,
    ...
    gdf_pois = find_cat(gdf_pois, categories,
                        dummy=get_dummy_cat, tags_for_cat=tags_for_cat)
...
def plot_POI_folium(place_latlong: np.array,
    ...
    # y = lat
    # x = long
    map = folium.Map(location=place_latlong,
...
def route_between_coordinates(streets: nx.classes.MultiDiGraph,
    ...
    # on peut aussi mettre travel_time
    route = ox.shortest_path(streets, orig, dest, weight=weight)
    if route == None:
...
def get_type_cuisine(amenities: gpd.GeoDataFrame, unique=True):
    ...
    if not unique:
        type_restaurants = {"type_cuisine": type_restaurants.keys(
        ), "nb_cuisine": type_restaurants.values}
    ...
    unique_type = {"type_cuisine": unique_type.keys(
    ), "nb_cuisine": unique_type.values()}
...
def get_POI_cat_on_INSPIRE_grid(url: str, city: str = "Paris", reduced_cat=True):
    ...
    if reduced_cat:
        ...
        osmgdf = get_place_POI(
            "Paris", categories=categories.keys(), tags_for_cat=categories)
...
def calculate_2SFCA_demand(gdf, weights_by_id,
    ...
    """
    def f(row): return calcule_individual_demand(
        row=row, weight_age=weight_age)
    demand = gdf.apply(f, axis='columns', raw=False)
    weighted_demand = weights_by_id.multiply(other=demand, axis=0)
    #print("debug for weighted_demand")
    #print(weighted_demand)
    # we have for each column i the series of individual demande j * weight ij
    # donne pour chaque carré sa zone de patientèle cad le nombre de personnes
    return weighted_demand.sum()
...
def calcule_individual_demand(row,
    ...
    for k in weight_age.keys():
        #print(f'row is{row[k]} weight_age is {weight_age[k]}')
        demand += row[k] * weight_age[k]
        #print('multiplied!')
    #print(f'demand for row {row} is {demand}')
    return demand
...
def calculate_2SFCA_accessibility_var(supply, demand, weights_by_id):
    ...
    #    ratio[i] = supply[i] / demand[i]
    #print(ratio)
    
    ratio = supply / demand
    #weighted_ratio = weights_by_id.drop(columns='id').multiply(other=ratio, axis=0)
    weighted_ratio = weights_by_id.multiply(other=ratio, axis=0)
    #print(weighted_ratio.sum())
...
def calculate_distanceband_weights(gdf, idCol="IdINSPIRE", geometryCol="geometry", threshold=1):
    ...
    w_db = weights.distance.DistanceBand.from_dataframe(
        gdf, threshold=threshold, binary=False, radius=radius, geom_col=geometryCol, ids=idCol)
    ...
    for i, id in enumerate(ids):
        weights_by_id = weights_by_id.join(
            pd.Series(index=ids, data=full_matrix[i], name=id))
...
def calculate_2SFCA_accessibility(gdf, interestsVar, weights_by_id, weight_age={
    ...
    demand = calculate_2SFCA_demand(
        gdf=gdf, weights_by_id=weights_by_id, weight_age=weight_age)
    ...
    print(demand)
    print("demand calculated!")
    
    def f(s): return calculate_2SFCA_accessibility_var(
        supply=s, demand=demand, weights_by_id=weights_by_id)
...
def aggregate_2SFCA(gdf,
    ...
    for i in categories:
        gdf[str("weight_" + i)] = gdf[i].sum() / \
            (gdf[categories].sum(axis=1).sum())

    if weight:
        for i in categories:
            gdf[str(i + "_access" + "_norm*weight")] = (gdf[str(i + "_access")].apply(lambda x: (x - gdf[str(i + "_access")
                                                                                                         ].min())/(gdf[str(i + "_access")].max() - gdf[str(i + "_access")].min())))*(1 - gdf[str("weight_" + i)])
        gdf["CS_aggregated"] = gdf[[
            i + "_access_norm*weight" for i in categories]].sum(axis=1)

    else:
        for i in categories:
            gdf[str(i + "_access" + "without_norm*weight")] = gdf[str(i + "_access")].apply(lambda x: (x -
                                                                                                       gdf[str(i + "_access")].min())/(gdf[str(i + "_access")].max() - gdf[str(i + "_access")].min()))
        gdf["CS_aggregated_without_weight"] = gdf[[
            i + "_accesswithout_norm*weight" for i in categories]].sum(axis=1)
...
def LM_test(df, dep_var, indep_var, w):
    ...
    print("LM error test p_value for " + str(dep_var) +
          " : " + str(round(lms.lme[1], 4)))
    print("LM lag test p_value for " + str(dep_var) +
          " : " + str(round(lms.lml[1], 4)))
    print("Robust LM error test p_value for " +
          str(dep_var) + " : " + str(round(lms.rlme[1], 4)))
    print("Robust LM lag test p_value for " +
          str(dep_var) + " : " + str(round(lms.rlml[1], 4)))
    print("LM SARMA test p_value for " + str(dep_var) +
          " : " + str(round(lms.sarma[1], 4)))
...
def compare_places(places: list,
    ...
    df = pd.DataFrame(
        {'Place': list_places, 'Var': list_var, 'Number': list_number})
