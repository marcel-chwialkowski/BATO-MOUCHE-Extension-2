...
def folium_color_map(gdf, var: str, cmap = 'Set1', discrete = False):
    if discrete:
        colors = colormaps[cmap](range(len(gdf[var].unique())))
        colors = ListedColormap(colors)
        cmap = colors
        m = gdf.explore(var, 
                        cmap=colors,
                        categorical = True,
                        style_kwds=dict(color='black', opacity=0.1, fillOpacity=0.4))
    else:
        m = gdf.explore(var, 
                        cmap=cmap,
                        style_kwds=dict(color='black', opacity=0.1, fillOpacity=0.4))
...
def violin_plots(df, var_distinguante, nb_cluster, dico_var,
    ...
    for i, var in enumerate(var_distinguante):
        ...
        x = cluster_column
        y = var
        ax = sns.violinplot(data=df, x=x, y=y, ax=ax,
                            order=order, palette=colors, box_visible=True)
    ...
    for i, var in enumerate(var_distinguante):
        ...
        for j in order:
            if len(df[df[cluster_column] == j][var].dropna()) > 1:
                to_do_stat_on.append(j)
        pairs = list(combinations(to_do_stat_on, r=2))
        if len(var_distinguante) > 2:
            ax = ax = axs[(i-(i % 2))//2][i % 2]
        else:
            ax = axs[i]
